...
## Adding or removing tickers — including pre-IPO / no-cashtag names

Open `collector.py` and edit the `QUERIES` list near the top. Each entry is
a `{label, query}` pair:
...
- **`label`** is what appears in the dashboard's search box and the key in
  `data.json`. Keep it short and recognizable.
- **`query`** is the actual X search string, using the same syntax as X's
  own search bar (quotes for exact phrases, `OR`, `-word` to exclude, etc.).
  - For a listed ticker, `"$SYMBOL"` (the cashtag) is usually enough.
  - For anything without a cashtag yet — pre-IPO companies, private
    companies people speculate about — search the name directly, and add a
    few OR'd context words to cut down on noise from unrelated uses of the
    same name (e.g. `"Unitree" (robot OR robotics OR IPO OR humanoid)`).
    Test the query in X's own search bar first to sanity-check what it
    actually returns before wiring it in here.

Each entry gets its own array in `data.json` automatically, and shows up in
the dashboard's search box the next time the collector runs — no changes
needed in `index.html`. The dashboard shows a `$` prefix automatically for
short all-caps labels (tickers) and no prefix for name-style labels.
