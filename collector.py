#!/usr/bin/env python3
"""
Mention + sentiment collector for X (Twitter).

Runs on a schedule (GitHub Actions cron, or any machine with cron/Task
Scheduler). Each run, for every entry in QUERIES:
  ...
  4. Appends one row to data.json (keyed by "label"), which the dashboard
     (index.html) reads.

Environment variables required:
  X_BEARER_TOKEN   - Bearer token from your X developer app (pay-per-use project)

Configure tracked names / cadence in CONFIG below.
"""
...
#
# Each entry needs:
#   label - what shows up in the dashboard's search box / data.json key
#   query - the actual X search string. Use a cashtag ("$UBER") for listed
#           tickers, or a plain keyword search (optionally narrowed with
#           OR'd context words) for anything without a cashtag yet — e.g.
#           pre-IPO companies. X search syntax applies here same as typing
#           into the X search bar: quotes for exact phrases, OR, -word to
#           exclude, etc.
# ---------------------------------------------------------------------------
...
def get_mention_count(query, bearer_token, hours_back):
    ...
    data = x_api_get(
        ...
        {"query": full_query, "start_time": start_time, "granularity": "hour"},
        bearer_token,
    )
...
def get_sample_texts(query, bearer_token, n):
    full_query = f'{query} -is:retweet lang:en'
    data = x_api_get(
        "/tweets/search/recent",
        {"query": full_query, "max_results": min(max(n, 10), 100), "tweet.fields": "text"},
        bearer_token,
    )
...
def main():
    ...
    for entry in QUERIES:
        label, query = entry["label"], entry["query"]
        try:
            count = get_mention_count(query, bearer_token, hours_back)
            sample = get_sample_texts(query, bearer_token, SAMPLE_SIZE)
        ...
        data.setdefault(label, []).append(row)
        print(f"{label}: {row}")
